...
if (!testDatabaseUrl) {
	throw new Error(
		"TEST_DATABASE_URL is required for portal-e2e. Set it in shell env or apps/backend/.env.test.",
	);
}

if (testDbSafe !== "true") {
	throw new Error(
		"TEST_DB_SAFE=true is required for portal-e2e to run against a test database safely (shell env or apps/backend/.env.test).",
	);
}
